...
class Launcher {
  ...
  async launch(options = {}) {
    ...
    firefoxProcess.on('message', (event) => {
      console.log('on(message)', event);
    })
    firefoxProcess.once('exit', (event) => {
      console.log('on(exit)', event);
    })
    firefoxProcess.on('error', (event) => {
      console.log('on(error)', event);
    })
    firefoxProcess.on('close', (event) => {
    ...
    const waitForFirefoxToClose = new Promise((fulfill, reject) => {
      firefoxProcess.once('exit', (error) => {
        console.log('once(exit)', error);
        firefoxClosed = true;
    ...
    function killFirefox() {
      ...
      if (firefoxProcess.pid && !firefoxProcess.killed && !firefoxClosed) {
        // Force kill firefox.
        console.log('Force kill firefox.');
        try {
          ...
          console.log('Unable to kill, the process might have already stopped', e);
        }
      }
...
function waitForWSEndpoint(firefoxProcess, timeout) {
  return new Promise((resolve, reject) => {
    ...
    const listeners = [
      ...
      helper.addEventListener(rl, 'close', () => {
        console.log('rl.close');
        onClose();
      }),
      helper.addEventListener(firefoxProcess, 'exit', (event) => {
        console.log('firefoxProcess.exit', event);
        onClose();
      }),
    ...
    function onClose(error) {
      ...
      reject(new Error([
        'Failed to launch Firefox!' + (error ? ' ' + error : ''),
        stderr,
        '',
      ].join('\n')));
    }
